...
def grapDATA(file, year, mon, day):
    ...
    PATH = "C:\Users\Wei\Documents\Other\Github\Portfolio\\"
    #PATH = "./"
    ...
    for k, name in enumerate(sorted(dict)):
        print k + 1, name
        ...
        s2 = "&amp;b="
        s3 = "&amp;c="
        s4 = "&amp;d=02&amp;e=31&amp;f=2017&amp;g=d&amp;ignore=.csv"
    ...
    for name in sorted(DATA):
        ...
        s += '\t' + name
    fout.write(s + '\n')
    for date in sorted(Date, reverse=True):
        s = date
        for name in sorted(DATA):
            if date not in DATA[name]:
                DATA[name][date] = '-'
...
if __name__ == "__main__":
    ...
    LIST = {'AAPL', 'M','AAL','KTOS','SBUX','MMM','GS','T','GM','FE','HRB','X','CDI','MAS','UNH', '%5EGSPC'}
    calMeanVar(DATA, Date, LIST)
    '''
    ALL = DATA.keys()
    print "ALL\t", ALL
    Choose = random.randint(15)
    Max_SP = 0
